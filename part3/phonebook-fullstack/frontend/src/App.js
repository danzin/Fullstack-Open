...
const App = () => {
  ...
  const createPerson = (personObj) => {
    ...
    .create(personObj)
      ...
      .catch(err => {
        ...
        showNotification('error', `error adding ${personObj.name} ` + err.response.data.error)
      }
      )
  ...
  const addPerson = (event) => {
    ...
    if(persons.some(checkName)){
      if(window.confirm(`${newName} already exists. Would you like to change the number?`) === true){
      ...
      console.log(persons.some(checkName))
      createPerson(personObj)
    }
  }
